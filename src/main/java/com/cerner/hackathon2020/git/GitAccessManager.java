...
public class GitAccessManager {
  ...
  /*
   * public static void main(String[] args) { SearchCriteria criteria = new SearchCriteria();
   * criteria.setRepositoryName(null);// "IPDev-Millennium-MessageCenter"
   * criteria.setKeyword("SOLM64");// SOLM64, BSDPATIENTA getSearchedResult(criteria);
  ...
  public static Map<String, List<FileDetails>> getSearchedResult(final SearchCriteria criteria) {
    ...
    try {
      ...
      // jsonStr = Obj.writeValueAsString(outputObject);
      // System.out.println(jsonStr);

    } catch (Exception e) {
      e.printStackTrace();
    }
    return outputObject;// jsonStr;
  }
}
