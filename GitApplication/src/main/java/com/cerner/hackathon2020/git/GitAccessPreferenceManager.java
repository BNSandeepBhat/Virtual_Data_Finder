 ...
 *
 */
public class GitAccessPreferenceManager {

  public static void main(String[] args) {
    SearchCriteria criteria = new SearchCriteria();
    criteria.setRepositoryName("IPDev-Millennium-MessageCenter");
    criteria.setKeyword("Internal Recipient Default");
    Map<String, List<FileDetails>> outputObject = getSearchedResult(criteria);
  }
    ...
    try {
      if (criteria.getRepositoryName() != null && !criteria.getRepositoryName().isEmpty()) {
        ...
        urlString = "https://github.cerner.com/api/v3/search/code?q=\"" + criteria.getKeyword()
            + "\"+in:file+extension:script+repo:" + repoFilterString;
      } else {
        urlString = "https://github.cerner.com/api/v3/search/code?q=\"" + criteria.getKeyword()
            + "\"+in:file+extension:script";
      }
